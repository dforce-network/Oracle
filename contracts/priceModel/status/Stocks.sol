...
contract Stocks is Base {
     ...
     */
    function _setMarketOpeningTime(uint256 _marketOpeningTime)
        public
        onlyOwner
     ...
     */
    function _setDuration(uint256 _duration)
        public
        onlyOwner
        isValidTime(_duration)
    {
    ...
    /**
     * @notice Called by the contract owner to pause, triggers stopped status.
    ...
    /**
     * @notice Called by the contract owner to pause an asset, triggers stopped status.
     * @dev Set assetPauseTime to the current timestamp.
     * @param _asset Asset for which to set the `assetPauseTime`.
     */
    function pauseAsset(address _asset) external onlyOwner {
        uint256 _oldAssetPauseTime;
        (_oldAssetPauseTime, assetPauseTime[_asset]) = (
            assetPauseTime[_asset],
            block.timestamp
     ...
     */
    function _getAssetStatus(address _asset, uint256 _timestamp)
        internal
        view
        virtual
        returns (bool)
     ...
     */
    function getDateInfo(uint256 _timestamp)
        external
        view
        returns (
            uint256,
