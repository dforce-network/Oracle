...
export async function verifyAllowError(
  value0: BigNumber,
  value1: BigNumber,
  errorFactor: number
) {
...
export function correctPrice(
  ...
  const X: BigNumber = ethers.utils.parseUnits("10", "wei");

  return price
    .mul(X.pow(ethers.utils.parseUnits("36", "wei").sub(assetDecimals)))
    .div(X.pow(priceDecimals));
}
